...
public class Pravougaonik extends Kvadrat {
	...
	public boolean sadrzi(int x, int y) {
		if (goreLevo.getX() <= x && goreLevo.getX() + stranica >= x && goreLevo.getY() <= y
				&& goreLevo.getY() + sirina >= y)
			return true;
		else
			return false;
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof Pravougaonik) {
			...
			if (pomocni.goreLevo.equals(goreLevo) && pomocni.sirina == sirina && pomocni.stranica == stranica)
				return true;
